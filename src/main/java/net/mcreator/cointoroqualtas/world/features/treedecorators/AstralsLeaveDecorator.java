...
public class AstralsLeaveDecorator extends LeaveVineDecorator {
	public static MapCodec<AstralsLeaveDecorator> CODEC = MapCodec.unit(AstralsLeaveDecorator::new);
	public static TreeDecoratorType<?> DECORATOR_TYPE = new TreeDecoratorType<>(CODEC);

	@SubscribeEvent
	public static void registerTreeDecorator(RegisterEvent event) {
		event.register(Registries.TREE_DECORATOR_TYPE, ResourceLocation.parse("cointoro_qualtas:astrals_tree_leave_decorator"), () -> DECORATOR_TYPE);
	}

	public AstralsLeaveDecorator() {
		super(0.25f);
	}
	...
	private static void addVine(BlockPos pos, Direction direction, TreeDecorator.Context context) {
		context.setBlock(pos, Blocks.POINTED_DRIPSTONE.defaultBlockState());
		int i = 4;
		for (BlockPos blockpos = pos.below(); context.isAir(blockpos) && i > 0; --i) {
			context.setBlock(blockpos, oriented(Blocks.POINTED_DRIPSTONE.defaultBlockState(), direction));
